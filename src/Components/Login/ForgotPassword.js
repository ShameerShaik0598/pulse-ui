...
function ForgotPassword() {
    ...
    const handleOtpForm = async(event) =>{
        let res=await axios.post("http://localhost:1000/user/forgot-password",event)
        console.log(res);
    ...
    const handleForgetPassword =async (passwordObject) =>{
        ...
        let res=await axios.post(`http://localhost:1000/user/reset-password/email/${emailOfUser}`,passwordObject)
